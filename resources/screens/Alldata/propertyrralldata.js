...
export default function Propertyrralldata({ navigation }) {
  const back = () => {
    navigation.goBack()
  }
  ...
  useEffect(() => {
    fetch('https://olx.devoa.xyz/api/pfr_cat')
      ...
      {isLoading ? <ActivityIndicator size={100} color="#0c0e87" style={{ marginTop: 200 }} /> :
        <View style={{ backgroundColor: "#0c0e87", position: "relative" }} >
          <View style={styles.head}>
            ...
            <Text style={{ color: "white", textAlign: "center", fontSize: 20 }} >Property For Rent All Ads</Text>
          </View>
                          ...
                          <View>
                            <Box border={1} style={{ backgroundColor: "white", borderRadius: 20, borderColor: "#010385" }}>
                              <VStack space={0} divider={<Divider />}>
                                <Box px={0} pt={0}>
                                  <Image style={{ width: 300, height: 150, borderTopLeftRadius: 20, borderTopRightRadius: 20 }} source={{ uri: i`https://olx.devoa.xyz/User/images/post_images/${item.main_img}`}} />
                                </Box>

                                <Box px={4} pt={3} style={{ flexDirection: "row" }}>
                                  <Text style={styles.te}>
                                    Prices :<Text style={styles.tes}> {item.price} </Text>
                                  </Text>
                                </Box>
                                <Box px={4} pt={3} style={{ flexDirection: "row" }}>
                                  <Text style={styles.te}>
