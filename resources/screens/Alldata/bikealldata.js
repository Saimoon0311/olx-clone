...
export default function Bikealldata({ navigation }) {
  const back = () => {
    navigation.goBack()
  }
  ...
  useEffect(() => {
    fetch('https://olx.devoa.xyz/api/bikes_cat')
  ...
  return (
    <View>
      {isLoading ? <ActivityIndicator size={100} color="#0c0e87" style={{ marginTop: 200 }} /> :
        <View style={{ position: "relative", backgroundColor: "#0c0e87" }} >
          <View style={styles.head}>
            <Ionicons name="arrow-undo-outline" color="white" onPress={back} size={24} style={{ paddingTop: 20, paddingLeft: 10 }} />
            <Text style={{ color: "white", textAlign: "center", fontSize: 20 }} >Bike Ads</Text>
          </View>
